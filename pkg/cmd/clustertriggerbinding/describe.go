...
`
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 0 {
				tb, err := clustertriggerbinding.GetAllClusterTriggerBindingNames(cs.Triggers, p.Namespace())
				if err != nil {
					return err
				}
				if len(tb) == 1 {
					opts.ClusterTriggerBindingName = tb[0]
				} else {
					err = askClusterTriggerBindingName(opts, tb)
					if err != nil {
						return err
					}
				}
